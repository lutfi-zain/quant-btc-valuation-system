...
- When the raw value is above `t_plus_1` (outside the defined bottom zone), the result SHALL be `0.0` (neutral)
...
- When the raw value is below `t_minus_1` (outside the defined top zone), the result SHALL be `0.0` (neutral)
...
- **THEN** the result SHALL be `0.0` (neutral — no top signal defined for this metric)
...
- **THEN** the result SHALL be `0.0` (neutral — above the +1SD threshold, no top zone defined)
...
- **THEN** the result SHALL be `0.0` (neutral — below the -1SD threshold, no bottom zone defined)
